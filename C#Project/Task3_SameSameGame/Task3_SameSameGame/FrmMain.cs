...
namespace Task3_SameSameGame
{
    public partial class FrmMain : Form
    {
        ...
        private void FindSameBlock(int nX, int nY)
        {
            ...
            if(nY > 0)
            {
                if((m_nBlock[nX , nY-1] == m_nPointBlock) && !m_bVisit[nX , nY-1])
                {
        ...
        private void MoveX()
        {
            ...
            for(i = 0; i < m_nSameCnt; i++)
            {
                ...
                if(nY == maxY-1)
                {
                    ...
                    for(j = 0; j < maxX-1; j++)
                    {
                        if(m_nBlock[j, maxY - 1] == -1 && m_nBlock[j + 1, maxY - 1] != -1)
                        {
                            nEmptyX = j;
                            if(nMinX > nEmptyX)
                            {
        ...
        private void FrmMain_MouseMove(object sender, MouseEventArgs e)
        {
            int i = 0;
            // 원본 그림을 표시한다.
            if (m_nSameCnt > 1 && m_nPointBlock != -1)
            {
                PictureBox pic;
                for (i = 0; i < m_nSameCnt; i++)
                {
                    pic = (System.Windows.Forms.PictureBox)panMain.Controls[m_nSel[i]];
                    pic.Image = imglstBlock.Images[m_nPointBlock];
                }
            }
        }
